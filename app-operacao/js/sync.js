...
// v2026-03-31 — Sync offline-first (REST ou Legacy), filas, auto-sync e wake automático do backend
...
const META = {
  lastWake: "backend_last_wake_v1",
  wakeLock: "backend_wake_lock_v1",
  apiMode: "api_mode_v1",
};

/* ================== exports usados no front ================== */
...
/* ================== wake automático do backend ==================
   Ideia:
   - Render pode dormir.
   - Antes de operações importantes, tentamos "acordar" o backend.
   - Evita chamadas simultâneas demais usando lock simples em localStorage.
=============================================================== */

const WAKE_INTERVAL_MS = 4 * 60 * 1000; // 4 minutos
const WAKE_LOCK_MS = 20 * 1000;         // trava curta para evitar spam
const WAKE_RETRY_DELAY_MS = 1800;

function getLastWakeTs() {
  return Number(localStorage.getItem(META.lastWake) || 0);
}

function setLastWakeTs(ts = _now()) {
  localStorage.setItem(META.lastWake, String(ts));
}

function getWakeLockTs() {
  return Number(localStorage.getItem(META.wakeLock) || 0);
}

function setWakeLockTs(ts = _now()) {
  localStorage.setItem(META.wakeLock, String(ts));
}

function clearWakeLockTs() {
  localStorage.removeItem(META.wakeLock);
}

function shouldWakeBackend() {
  const last = getLastWakeTs();
  return _now() - last > WAKE_INTERVAL_MS;
}

async function pingBackend(timeout = 8000) {
  try {
    await httpJSON(`/clientes`, { method: "GET", timeout });
    return true;
  } catch (e) {
...
export async function wakeBackend({ force = false } = {}) {
  ...
  if (!force && !shouldWakeBackend()) {
    return true;
  }

  const currentLock = getWakeLockTs();
  if (currentLock && _now() - currentLock < WAKE_LOCK_MS) {
    return false;
  }
  ...
  try {
    let ok = await pingBackend(8000);

    if (!ok) {
      await _sleep(WAKE_RETRY_DELAY_MS);
      ok = await pingBackend(10000);
    }
...
/* ================== detecção de modo ================== */
function getApiMode() {
  return window.API_MODE || localStorage.getItem(META.apiMode) || "auto";
}

function setApiMode(m) {
  window.API_MODE = m;
  localStorage.setItem(META.apiMode, m);
}
...
export async function pullClientes() {
  ...
  try {
    if (mode === "rest") {
      const data = await httpJSON(`/clientes`, { method: "GET" });
    ...
    const docs = arr.map((n) => ({
      ...
      nome: low(n.nome),
      endereco: low(n.endereco),
      bairro: low(n.bairro),
      rota: low(n.rota),
      pctCliente: Number(n.pctCliente) || 0,
      negativo: Number(n.negativo) || 0,
      recuperacao: Number(n.recuperacao) || 0,
    }));

    localStorage.setItem(K.clientes, JSON.stringify(docs));
    return { ok: true, count: docs.length, mode: "legacy" };
  } catch (e) {
    if (e?.status === 404 && mode === "rest") {
      setApiMode("legacy");
      return pullClientes();
    }
    throw e;
...
export async function pushClientes({ upserts = [], deletes = [] }) {
  ...
  try {
    if (mode === "rest") {
      return await httpJSON(`/clientes/bulk`, {
        method: "POST",
    ...
    for (const it of upserts) {
      await httpJSON(`/`, {
        method: "POST",
        body: { acao: "cadastrarCliente", dados: it },
      });
    }
    ...
    if (e?.status === 404 && mode === "rest") {
      setApiMode("legacy");
      return pushClientes({ upserts, deletes });
    }
    throw e;
  }
}
...
export async function pushVales({ upserts = [], deletes = [] }) {
  ...
  try {
    if (mode === "rest") {
      return await httpJSON(`/vales/bulk`, {
        method: "POST",
        body: { upserts, deletes },
      });
    }
    ...
    for (const it of upserts) {
      await httpJSON(`/`, {
        method: "POST",
        body: { acao: "cadastrarVale", dados: it },
      });
    }
    ...
    if (e?.status === 404 && mode === "rest") {
      setApiMode("legacy");
      return pushVales({ upserts, deletes });
    }
    throw e;
  }
}
...
export async function pushMaquinas({ upserts = [], deletes = [] }) {
  ...
  try {
    if (mode === "rest") {
      return await httpJSON(`/maquinas/bulk`, {
        method: "POST",
        body: { upserts, deletes },
      });
    }
    ...
    for (const it of upserts) {
      await httpJSON(`/`, {
        method: "POST",
        body: { acao: "cadastrarMaquina", dados: it },
      });
    }
    ...
    if (e?.status === 404 && mode === "rest") {
      setApiMode("legacy");
      return pushMaquinas({ upserts, deletes });
    }
    throw e;
  }
}
...
export function queueCliente(item) {
  ...
  const i = q.findIndex(
    (op) =>
      op.type === "upsert" &&
      (
        (op.item?.id && item?.id && op.item.id === item.id) ||
        low(op.item?.nome) === keyNome
      )
  );

  if (i >= 0) {
    q[i] = { type: "upsert", item: { ...q[i].item, ...item } };
  } else {
...
export function queueVale(item) {
  ...
  const i = q.findIndex(
    (op) => op.type === "upsert" && op.item?.id && item?.id && op.item.id === item.id
  );
...
export async function processarFila() {
  ...
  // CLIENTES
  {
    ...
    for (const op of q) {
      try {
        if (op.type === "upsert") {
          await pushClientes({ upserts: [op.item], deletes: [] });
        } else if (op.type === "delete") {
          await pushClientes({ upserts: [], deletes: [op.id] });
        }
      } catch {
        keep.push(op);
      }
  ...
  // VALES
  {
    ...
    for (const op of q) {
      try {
        if (op.type === "upsert") {
          await pushVales({ upserts: [op.item], deletes: [] });
        } else if (op.type === "delete") {
          await pushVales({ upserts: [], deletes: [op.id] });
        }
      } catch {
        keep.push(op);
      }
  ...
  // MÁQUINAS
  {
    ...
    for (const op of q) {
      try {
        if (op.type === "upsert") {
          await pushMaquinas({ upserts: [op.item], deletes: [] });
        } else if (op.type === "delete") {
          await pushMaquinas({ upserts: [], deletes: [op.id] });
        }
      } catch {
        keep.push(op);
      }
