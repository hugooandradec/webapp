...
function salvarNoStorage(){
  ...
  localStorage.setItem("valorFinalLancamento", document.getElementById("valorFinal")?.value || "");
}
function carregarDoStorage(){
  const d = document.getElementById("data");
  const vi = document.getElementById("valorInicial");
  const vf = document.getElementById("valorFinal");
  if (d)  d.value  = localStorage.getItem("dataLancamento") || "";
  if (vi) vi.value = localStorage.getItem("valorInicialLancamento") || "";
  if (vf) vf.value = localStorage.getItem("valorFinalLancamento") || "";
...
function atualizarTotais(){
  ...
  const valorDecl     = parseValor(document.getElementById("valorFinal").value);
  const totalNaMao    = valorInicial + totalDinheiro + totalOutros - totalSaida;
  const dif           = valorDecl - totalNaMao;
  ...
  r.innerHTML = `
    ...
    <p><strong>Valor Inicial:</strong> <span style="color:${corPos}">${formatarMoeda(valorInicial)}</span></p>
    ...
    <p><strong>Valor Informado:</strong> <span style="color:${valorDecl<0?corNeg:corPos}">${formatarMoeda(valorDecl)}</span></p>
    <p><strong>Valor Total:</strong> <span style="color:${totalNaMao<0?corNeg:corPos}">${formatarMoeda(totalNaMao)}</span></p>
    <p><strong>Diferença:</strong> <span style="color:${dif===0?"#000":(dif<0?corNeg:corPos)}">${formatarMoeda(dif)}</span></p>`;
  salvarNoStorage();
}
...
window.visualizarRelatorio = function () {
  ...
  const valorInformado = parseValor(document.getElementById("valorFinal")?.value || 0);
  ...
  const diferenca     = valorInformado - valorTotal;
  ...
  const linhas = listaLancamentos
    .map(e=>{
      const sub = (Number(e.dinheiro)||0) + (Number(e.outros)||0) - (Number(e.saida)||0);
      return `<tr>
        <td style="text-transform:capitalize">${e.ponto}</td>
  ...
  const html = `
    ...
    <div style="font-family: Arial; font-size:14px; padding-bottom:70px;">
      ...
      <div style="margin:0 0 10px;">
        ...
        <p><strong>Valor Inicial:</strong> <span style="color:${corPos}">${formatarMoeda(valorInicial)}</span></p>
        <p><strong>Valor Informado:</strong> <span style="color:${corPos}">${formatarMoeda(valorInformado)}</span></p>
        <p><strong>Valor Total:</strong> <span style="color:${corPos}">${formatarMoeda(valorTotal)}</span></p>
        <p><strong>Diferença:</strong> <span style="color:${diferenca===0?'#000':(diferenca<0?corNeg:corPos)}">${formatarMoeda(diferenca)}</span></p>
      </div>
...
window.limparLancamentos = function(){
  ...
  localStorage.removeItem("valorFinalLancamento");
